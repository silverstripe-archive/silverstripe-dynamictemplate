...
class DynamicTemplate extends Folder
{
    ...
    public static $singular_name = "Dynamic Template";

    public static $plural_name = "Dynamic Templates";
    ...
    public static function set_dynamic_template_folder($value)
    {
        self::$dynamic_template_folder = $value;
    }

    public static function get_dynamic_template_folder()
    {
        return self::$dynamic_template_folder;
    }

    public static function get_dynamic_template_folder_object()
    {
     ...
     */
    public static function import_file($file, &$errors, $altName = "")
    {
        // Create the holder
        ...
        if (!$template = self::extract_file($file, $holder, $errors, $altName)) {
            return null;
        }
    ...
    public static function extract_bundle($file, &$errors)
    {
        // Create the holder
        $holder = DataObject::get_one("Folder", "\"Filename\"='assets/" . self::$dynamic_template_folder . "'");
        if (!$holder) {
        ...
        if (!$template = self::extract_file($file, $holder, $errors)) {
            return null;
        }
     ...
     */
    public static function extract_file($compressedFile, $holder, &$errors, $altName)
    {
     ...
     */
    public function flushManifest($manifest)
    {
     ...
     * @return void
     */
    public function sanitise($manifest)
    {
    ...
    public function generateManifest()
    {
        ...
        if (count($templates) > 0) {
            $manifest->addPath('index', $templates[0], 'main');
        }

        $css = $this->getFilesInDirByExt("css", ".css");
        if ($css != null) {
            foreach ($css as $c) {
                $manifest->addPath('index', $c);
            }
        }
        $js = $this->getFilesInDirByExt("javascript", ".js");
        if ($js != null) {
            foreach ($js as $j) {
                $manifest->addPath('index', $j);
            }
        }
        return $manifest;
    }
     ...
     * returned.
     */
    public function getFilesInDirByExt($subdir, $ext)
    {
        $paths = array();
     ...
     *							are none.
     */
    public function loadManifestFile($file, &$errors)
    {
        $manifest = new DynamicTemplateManifest();
...
//		if(file_exists("{$dir}/{$filename}")) throw new Exception("file $filename already exists in the template");
    ...
    protected function getSubdirByExtension($extension, $editable = false)
    {
        switch ($extension) {
            ...
            case ".gif":
                if ($editable) {
                    throw new Exception("File type $extension is not supported as an editable file at the moment");
                }
                $subdir = "images";
                break;
            case ".css":
    ...
    public function addUploadToFolder($tmpFile)
    {
        ...
        while (file_exists("{$dir}/{$fileSansExt}{$ext}")) {
            ...
            if ($oldFile == $fileSansExt && $i > 2) {
                user_error("Couldn't fix $fileSansExt$ext with $i", E_USER_ERROR);
            }
        }
        ...
        if (move_uploaded_file($tmpFile['tmp_name'], "{$dir}/{$fileSansExt}{$ext}")) {
            ...
            if (!file_exists($tmpFile['tmp_name'])) {
                ...
                user_error("Folder::addUploadToFolder: Couldn't copy '$tmpFile[tmp_name]' to '{$dir}/{$fileSansExt}{$ext}'", E_USER_ERROR);
            }
            return false;
        }
     ...
     */
    public function addFileToManifest($path)
    {
        $manifest = $this->getManifest();
     ...
     */
    public function constructChildInFolder($name, $subFolder)
    {
...
class DynamicTemplateManifest
{
     ...
     */
    public static $extMap = array(".css" => "css", ".js" => "javascript", ".ss" => "templates");
     ...
     */
    public function hasPath($path, $action = null)
    {
    ...
    public function addPath($action, $path, $extra = null)
    {
        ...
        if (!isset($this->actions[$action][$section])) {
            $this->actions[$action][$section] = array();
        }
     ...
     * @returns void
     */
    public function removePath($action, $path)
    {
        // First check if this type of file is even recorded in the
        // manifest.
        $extension = self::get_extension($path);
        ...
        $section = DynamicTemplateManifest::$extMap[$extension];
        if (!isset($this->actions[$action]) || !isset($this->actions[$action][$section])) {
            return;
        }
    ...
    public function loadFromFile($file)
    {
        ...
        if (count($this->actions) == 0) {
            $errors[] = "There are no actions in the template's manifest";
        }
     ...
     * @return void
     */
    public function parseMetadata($def)
    {
    ...
    {
        if (!isset($this->actions[$action]["templates"])) {
            return;
        }
        ...
        if ($oldId >= 0) {
            $this->actions[$action]['templates'][$oldId]['type'] = "";
        }
     ...
     * @return void
     */
    public function getTemplatesForRendering($action, $dynamicTemplate)
    {
        $result = array();
        if ($this->actions["index"]["templates"]) {
            foreach ($this->actions["index"]["templates"] as $template) {
    ...
    public function getCssForRendering($action, $dynamicTemplate)
    {
        $result = array();
        if (isset($this->actions[$action]['css'])) {
            foreach ($this->actions[$action]['css'] as $css) {
                if ($css["linked"]) {
                    $path = $css["path"];
                } else {
    ...
    public function getJavascriptForRendering($action, $dynamicTemplate)
    {
        $result = array();
        if (isset($this->actions[$action]['javascript'])) {
            foreach ($this->actions[$action]['javascript'] as $js) {
                if ($js["linked"]) {
                    $path = $js["path"];
                } else {
...
class DynamicTemplateUploadField extends UploadField
{
    ...
    public function upload(SS_HTTPRequest $request)
    {
        ...
        if (!$templateId || !is_numeric($templateId)) {
            $return['error'] = _t("DynamicTemplate.TEMPLATENOTSET", "Could not identify the dynamic template");
        }

        if (!$return['error']) {
            $template = DataObject::get_by_id('DynamicTemplate', $templateId);
            if (!$template) {
                $return['error'] = _t("DynamicTemplate.TEMPLATENOTFOUND", "Could not find dynamic template");
            }
        }
